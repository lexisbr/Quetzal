            ...
            if (tipoValor == Tipo.STRING) 
            {
                if(Number(valor) != NaN)
                {
                    if (this.tipoParse === Tipo.INT) {
                        ...
                        {
                            return new Excepcion(this.linea, this.columna, "Error Semantico", "No es posible convertir a Boolean la cadena ingresada");
                        }
                    } else {
                        return new Excepcion(this.linea, this.columna, "Error Semantico", "La Funcion Parse no existe para este tipo de dato");
                    }
                } else {
                        return new Excepcion(this.linea, this.columna, "Error Semantico", "Cadena Erronea para Funcion Parse, solo permite numeros");

                }
            } else {
                
                return new Excepcion(this.linea, this.columna, "Error Semantico", "Tipo de Dato Erroneo para Funcion Parse, la Expresion no es de Tipo String");
